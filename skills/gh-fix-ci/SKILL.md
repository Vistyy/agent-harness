---
name: "gh-fix-ci"
description: "Use when a user asks to debug or fix failing GitHub PR checks that run in GitHub Actions. Use `gh` for PR metadata, patch context, Actions check status, and log inspection before implementing any approved fix."
---
...
Use this skill when the task is specifically about failing GitHub Actions checks on a pull request. This workflow is CLI-first:

- Use `gh` for PR metadata, changed files, review context, GitHub Actions checks, and logs.
...
2. Resolve the PR.
   ...
   - When repo and PR are known, fetch PR metadata and patch context through `gh`.
...
- Use `gh` for Actions log retrieval.
